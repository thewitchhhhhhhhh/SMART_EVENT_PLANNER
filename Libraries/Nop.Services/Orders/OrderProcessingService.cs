...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
            ...
            ProcessPaymentResult processPaymentResult, PlaceOrderContainer details)
        {
            ...
            var availabilityDate = await _productService
                .GetProductAvailabilityDateByIdAsync(
                details.Cart.FirstOrDefault(x => x.ProductAvailabilityDateId.HasValue)?.ProductAvailabilityDateId.GetValueOrDefault()??default);

            order.CreatedOnUtc = availabilityDate?.AvailableDate ?? order.CreatedOnUtc;
        ...
        protected virtual async Task MoveShoppingCartItemsToOrderItemsAsync(PlaceOrderContainer details, Order order)
        {
            foreach (var sc in details.Cart)
            {
                ...
                var orderItem = new OrderItem
                {
                    ...
                    ProductAvailabilityDateId = sc.ProductAvailabilityDateId,
                };
        ...
        protected virtual async Task MarkDateAsBookedAsync(Order order)
        {
            if (order.PaymentStatus == PaymentStatus.Paid)
            {
                //update hall booking date to booked
                var orderItems = await _orderService.GetOrderItemsAsync(orderId: order.Id);
                var productAvailability = await _productService.GetProductAvailabilityDateByIdAsync(orderItems.FirstOrDefault(oi => oi.ProductAvailabilityDateId.HasValue)?.ProductAvailabilityDateId ?? 0);
                if (productAvailability is not null)
                {
                    productAvailability.IsBooked = true;
                    await _productService.UpdateProductAvailabilityDateAsync(productAvailability);
                }
            }
        }
        ...
        public virtual async Task<PlaceOrderResult> PlaceOrderAsync(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            try
            {
                ...
                if (processPaymentResult.Success)
                {
                    ...
                    await MarkDateAsBookedAsync(order);
                }
                else
