    ...
    public partial class ShoppingCartService : IShoppingCartService
    {
            ...
            string checkoutAttributesXml, bool validateCheckoutAttributes)
        {
            ...
            foreach (var sci in shoppingCart)
            {
                ...
                if ((await _productService.GetAllProductAvailabilityDatesAsync(productId: sci.ProductId, isBooked: true))
                    .Any(x => x.Id.Equals(sci.ProductAvailabilityDateId.GetValueOrDefault())))
                {
                    warnings.Add("Selected has been reserved by any other customer. Please select another date.");
                    return warnings;
                }
            ...
            bool includeDiscounts)
        {
            ...
            return await GetUnitPriceAsync(product,
                ...
                includeDiscounts,
                shoppingCartItem.ProductAvailabilityDateId);
        }
        ...
        public virtual async Task<(decimal unitPrice, decimal discountAmount, List<Discount> appliedDiscounts)> GetUnitPriceAsync(Product product,
            ...
            bool includeDiscounts,
            int? ProductAvailabilityDateId = default)
        {
            if (product == null)
                throw new ArgumentNullException(nameof(product));
            ...
            else
            {
                ...
                else
                {
                    ...
                    finalPrice += (await _productService.GetProductAvailabilityDateByIdAsync(ProductAvailabilityDateId.GetValueOrDefault()))?.Price ?? default;
                }
            }
        ...
        public virtual async Task<IList<string>> AddToCartAsync(Customer customer, Product product,
            ...
            decimal customerEnteredPrice = decimal.Zero,
            DateTime? rentalStartDate = null, DateTime? rentalEndDate = null,
            int quantity = 1, bool addRequiredProducts = true, int? productAvailabilityDateId = default)
        {
            if (customer == null)
                throw new ArgumentNullException(nameof(customer));
            ...
            //don't add multiple items or add quantity to previous if the hall is adding in the cart
            if (productAvailabilityDateId.HasValue &&
                cart.Any(x => x.ProductAvailabilityDateId.HasValue))
            {
                foreach (var cartItem in cart.Where(x => x.ProductAvailabilityDateId.HasValue))
                    await DeleteShoppingCartItemAsync(cartItem);
            ...
            else
            {
                ...
                shoppingCartItem = new ShoppingCartItem
                {
                    ...
                    ProductAvailabilityDateId = productAvailabilityDateId,
                };
