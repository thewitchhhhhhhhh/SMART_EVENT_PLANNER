...
namespace Nop.Services.Catalog
{
    ...
    public partial class ProductService : IProductService
    {
        ...
        private readonly IRepository<ProductAvailabilityDate> _productAvailabilityDateRepository;
        private readonly IRepository<Location> _locationRepository;
        private readonly IRepository<FoodType> _foodTypeRepository;
        ...
        public ProductService(CatalogSettings catalogSettings,
            ...
            IRepository<ProductAvailabilityDate> productAvailabilityDateRepository,
            IRepository<Location> locationRepository,
            ICategoryService categoryService,
            IRepository<FoodType> foodTypeRepository)
        {
            ...
            _productAvailabilityDateRepository = productAvailabilityDateRepository;
        ...
        public virtual async Task<IPagedList<Product>> SearchProductsAsync(
            ...
            int[] ids = default,
            int areaId = default,
            int[] foodTypeIds = default,
            DateTime? availableDate = default,
            decimal? availableDateMaxPrice = default,
            int guest = default)
        {
            ...
            if (!areaId.Equals(default))
                productsQuery = productsQuery.Where(p => p.AreaId.Equals(areaId));

            if (foodTypeIds?.Any() ?? false)
                productsQuery = productsQuery.Where(p => foodTypeIds.Contains(p.FoodTypeId));

            if (((!categoryIds?.Contains(_catalogSettings.CatererId)) ?? false) &&
                ((availableDate != null && availableDate.HasValue) || availableDateMaxPrice != null && availableDateMaxPrice.HasValue))
                productsQuery = (from p in productsQuery
                                join date in _productAvailabilityDateRepository.Table on p.Id equals date.ProductId
                                where !date.IsBooked &&
                                   (!availableDate.HasValue || date.AvailableDate.Date.Equals(availableDate.Value.Date)) &&
                                   (!availableDateMaxPrice.HasValue || date.Price <= availableDateMaxPrice.Value)
                                select p).Distinct();

            if (!guest.Equals(default))
            {
                if (categoryIds?.Contains(_catalogSettings.CatererId) ?? false)
                {
                    if (!availableDateMaxPrice.Equals(default))
                    {
                        productsQuery = productsQuery.Where(p => ((guest / p.PricePerRangeOfGuest) * p.Price) <= availableDateMaxPrice.Value);
                    }
                }
                else
                {
                    productsQuery = productsQuery.Where(p => p.Guest >= guest);
                }
            }
        ...
        public virtual async Task<IPagedList<ProductAvailabilityDate>> GetAllProductAvailabilityDatesAsync(int productId = default,
            DateTime availableDate = default,
            bool? isBooked = default,
            int vendorId = default,
            int pageSize = int.MaxValue,
            int pageIndex = 0)
        {
            return await _productAvailabilityDateRepository.GetAllPagedAsync(query =>
            {
                if (!productId.Equals(default))
                    query = query.Where(x => x.ProductId.Equals(productId));

                if (!availableDate.Equals(default))
                    query = query.Where(x => x.AvailableDate.Equals(availableDate));

                if (isBooked.HasValue)
                    query = query.Where(x => x.IsBooked.Equals(isBooked.Value));

                if (!vendorId.Equals(default))
                    query = from pad in query
                            join p in _productRepository.Table on pad.ProductId equals p.Id
        ...
        public virtual async Task<ProductAvailabilityDate> GetProductAvailabilityDateByIdAsync(int productAvailabilityDateId)
        {
            return await _productAvailabilityDateRepository.GetByIdAsync(productAvailabilityDateId,
                cache => cache.PrepareKeyForShortTermCache(NopEntityCacheDefaults<ProductAvailabilityDate>.ByIdCacheKey, productAvailabilityDateId));
        }

        public virtual async Task InsertProductAvailabilityDateAsync(ProductAvailabilityDate productAvailabilityDate)
        {
            if (productAvailabilityDate is null)
                throw new ArgumentNullException(nameof(productAvailabilityDate));

            await _productAvailabilityDateRepository.InsertAsync(productAvailabilityDate);
        }

        public virtual async Task UpdateProductAvailabilityDateAsync(ProductAvailabilityDate productAvailabilityDate)
        {
            if (productAvailabilityDate is null)
                throw new ArgumentNullException(nameof(productAvailabilityDate));

            await _productAvailabilityDateRepository.UpdateAsync(productAvailabilityDate);
        }

        public virtual async Task DeleteProductAvailabilityDateAsync(ProductAvailabilityDate productAvailabilityDate)
        {
            if (productAvailabilityDate is null)
                throw new ArgumentNullException(nameof(productAvailabilityDate));

            await _productAvailabilityDateRepository.DeleteAsync(productAvailabilityDate);
        }

        public virtual async Task<decimal> GetAvailabilitiesAverageAsync(Product product, bool? isBooked = default)
        {
            var allProductAvailabilityDates = await this.GetAllProductAvailabilityDatesAsync(productId: product.Id, isBooked: isBooked);
            return Convert.ToDecimal(allProductAvailabilityDates.Any() ? allProductAvailabilityDates.Average(x => x.Price) : default);
        }
        ...
        public virtual async Task<Location> GetLocationByIdAsync(int locationId)
        {
            return await _locationRepository.GetByIdAsync(locationId, cache => default);
        }

        public virtual async Task<IList<Location>> GetAllLocationAsync()
        {
            return await _locationRepository.GetAllAsync(query => query);
        }
        ...
        public virtual async Task<FoodType> GetFoodTypeByIdAsync(int foodTypeId)
        {
            return await _foodTypeRepository.GetByIdAsync(foodTypeId, cache => default);
        }

        public virtual async Task<IList<FoodType>> GetAllFoodTypeAsync()
        {
            return await _foodTypeRepository.GetAllAsync(query => query);
        }

        #endregion

        public virtual async Task<IList<Product>> GetAllProductsByVendorIdAsync(int vendorId)
        {
            return await _productRepository.GetAllAsync(query => query.Where(p => p.VendorId == vendorId));
        }

        public virtual async Task<bool> IsHallAsync(Product product)
        {
            if (product is null)
                return false;

            return new[] { _catalogSettings.BallroomId, _catalogSettings.RooftopId, _catalogSettings.BanquetId }
                            .Contains((await _categoryService.GetProductCategoriesByProductIdAsync(product.Id)).Select(x => x.CategoryId).FirstOrDefault());
        }
